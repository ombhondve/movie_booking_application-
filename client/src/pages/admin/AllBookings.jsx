              ...
              <tbody>
                {bookings.map((b, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-smoke">
                      {b.seatNumbers?.length > 0 ? b.seatNumbers.join(', ') : b.seatsBooked}
                    </td>
                    <td className="px-4 py-3">
                      ...
                      >
                        {b.status === 'cancelled' && b.cancelledBy ? `cancelled (${b.cancelledBy})` : b.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-smoke">
                      {b.rescheduled && (
                        <span className="rounded-full border border-amber-500/40 bg-amber-500/10 px-2 py-0.5 text-amber-300">
                          Rescheduled{b.previousShowDate ? ` from ${b.previousShowDate} ${b.previousShowTime}` : ''}
                        </span>
                      )}
                    </td>
                  </motion.tr>
