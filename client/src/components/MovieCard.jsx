...
import { Clock, Languages, Heart } from 'lucide-react';
...
export default function MovieCard({ movie, isFavorite = false, onToggleFavorite }) {
  ...
  return (
    <motion.button
      ...
      className="group relative flex flex-col overflow-hidden rounded-xl border border-ink-line bg-ink-raised text-left"
    >
      {onToggleFavorite && (
        <button
          onClick={(e) => {
          ...
          className="absolute right-2 top-2 z-10 inline-flex items-center gap-1 rounded-full bg-ink/80 px-2 py-1 text-xs backdrop-blur-sm transition-colors hover:bg-ink"
        >
          <Heart
            size={14}
            className={isFavorite ? 'fill-velvet text-velvet' : 'text-paper'}
          />
        </button>
      )}
